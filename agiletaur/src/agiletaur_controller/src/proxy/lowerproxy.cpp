...
lowerproxy::lowerproxy(std::string name) : Node(name){
    ...
    joint0_velocity_publisher = this->create_publisher<std_msgs::msg::Float64MultiArray>
        ("/joint0_velocity_controller/commands", 10);
    joint1_velocity_publisher = this->create_publisher<std_msgs::msg::Float64MultiArray>
        ("/joint1_velocity_controller/commands", 10);
...
void lowerproxy::PublishControlCommand(){
    ...
    message.data.push_back(10.0);
    RCLCPP_INFO(this->get_logger(), "Publishing: '%f'", message.data.at(0));
    joint0_velocity_publisher->publish(message);
    joint1_velocity_publisher->publish(message);
