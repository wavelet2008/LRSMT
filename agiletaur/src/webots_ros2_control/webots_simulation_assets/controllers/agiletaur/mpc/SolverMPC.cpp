...
void ct_ss_mats(Matrix<fpt,3,3> I_world, fpt m, Matrix<fpt,3,4> r_feet, Matrix<fpt,3,3> R_yaw, Matrix<fpt,13,13>& A, Matrix<fpt,13,12>& Bm)
{
  A.setZero();
  A(3,9) = 1.f;
  A(4,10) = 1.f;
  ...
  for(s16 b = 0; b < 4; b++)
  {
    Bm.block(6,b*3,3,3) = cross_mat(I_inv,r_feet.col(b));
    Bm.block(9,b*3,3,3) = Matrix<fpt,3,3>::Identity() / m;
  }
}
