...
//--------------------------------VMC库宏定义----------------------------------
#define EN_ADRC_ROLL          1
#define EN_ADRC_PITCH         1

#define EN_PLAN_USE_JERK      0   //使用Jerk轨迹速度慢
#define JEKR_USE_REAL         1
#define USB_SW_CAPTURE		  1	//使用MIT Capture Point的落足 前向会发散
#define USE_FORCE_REAL_GROUND 0   //TORT不会抖  0则高度保证但有姿态误差会明显大抖 为0横滚会控制不住
#define EN_SW_NEW             (USE_FORCE_REAL_GROUND&&0)||0   //USE_FORCE_REAL_GROUND只能使用这个新规划 另外不能使用USB_SW_CAPTURE
#define USE_QP                1
#define FIX_ST_TIME           1   //固定支撑时间
#define MIN_ST_TIME_RATE      0.55 //%
#define SW_LOW_RATE           0.55 //% 能有效解决倒了顶不起来
#define SW_TD_OVER_TIME       1   //s
#define TD_NO_SHOCK_TIME      0.01//s

#define FORCE_FB_USE_REAL     0   //足底力使用真实IQ轴估计

#define EN_ONLINE_SW_PLAN     1   //在线修正X方向落足点
#define RE_PLAN_DT            0.005//s越短越好

#define F_CONTROL_WITH_ROLL1   1   //补偿横滚力的输出  仿真里没区别
#define F_CONTROL_WITH_ROLL   0   //补偿横滚力的输出  仿真里没区别
#define F_EST_WITH_ROLL       0   //估计力使用横滚补偿
#define F_SWING_WITH_ROLL     1
#define ROLL_LIMIT_COM        25

#define KIN_5LINE_FK          1
#define KIN_5LINE_IK          0  //Bug
#define KIN_5LINE_J           1  //相比老的不太行
...
#define EN_TORT_LOAD_FORCE4 	0   //TROT对角等待使用LOAD力 否则着地直接使用力分配  导致侧翻无法复原！！！ 不能与GROUND_AFTER_TRIG一起使用
...
#define G_EST_TIME            0.1 //10Hz
#define DEAD_G_ATT            1.6 //degree
...
#define L3w          0.028	//3连杆
#define Mw           3.9  //kg     总重量
...
typedef struct
{
	...
	END_POS tar_epos_h, tar_epos_h_reg, tar_epos, tar_epos_n, tar_spd, tar_pos, tar_spd_sw;
	...
	char ground, ground_s, ground_noshock, is_touch;
...
typedef struct
{
	...
	float tar_att[3], tar_att_rate[3], tar_att_off[3], tar_att_bias[3], ground_off[2], tar_pos_off[3];
	//------------------------------------------
	float kp_trig[2];
	...
	//
	float att_trig[4], att_ctrl[4], att_rate_trig[3], att_rate_ctrl[3], att[3];
...
void get_license_moco(void);
char check_lisence_moco(int your_key0, int your_key1, int your_key2);
typedef struct {
	char en_record;
